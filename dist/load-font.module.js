...
const base64 = __webpack_require__(191)
...
let Mime = __webpack_require__(146);
module.exports = new Mime(__webpack_require__(415), __webpack_require__(14));
...
;// CONCATENATED MODULE: ./src/parse-attribs.js
...
function parseAttributes(obj) {
  ...
  for (var k in obj) {
    if (k === 'face' || k === 'charset') continue;else if (k === 'padding' || k === 'spacing') obj[k] = parse_attribs_parseIntList(obj[k]);else obj[k] = parseInt(obj[k], 10);
  }

  return obj;
}

function parse_attribs_parseIntList(data) {
  return data.split(',').map(function (val) {
    return parseInt(val, 10);
  });
}
...
;// CONCATENATED MODULE: ./src/parse-bmfont-binary.js
var HEADER = [66, 77, 70];
function readBMFontBinary(buf) {
  if (buf.length < 6) throw new Error('invalid buffer length for BMFont');
  var header = HEADER.every(function (_byte, i) {
    return buf.readUInt8(i) === _byte;
  });
...
function readChars(buf, i, blockSize) {
  ...
  for (var c = 0; c < count; c++) {
    var _char = {};
    var off = c * 20;
    _char.id = buf.readUInt32LE(i + 0 + off);
    _char.x = buf.readUInt16LE(i + 4 + off);
    _char.y = buf.readUInt16LE(i + 6 + off);
    _char.width = buf.readUInt16LE(i + 8 + off);
    _char.height = buf.readUInt16LE(i + 10 + off);
    _char.xoffset = buf.readInt16LE(i + 12 + off);
    _char.yoffset = buf.readInt16LE(i + 14 + off);
    _char.xadvance = buf.readInt16LE(i + 16 + off);
    _char.page = buf.readUInt8(i + 18 + off);
    _char.chnl = buf.readUInt8(i + 19 + off);
    chars[c] = _char;
  }
...
;// CONCATENATED MODULE: ./src/is-binary.js
 // for use with browserify

var is_binary_HEADER = buffer/* Buffer.from */.lW.from([66, 77, 70, 3]);

function equal(a, b) {
  if (!buffer/* Buffer.isBuffer */.lW.isBuffer(a)) return undefined;
  if (!buffer/* Buffer.isBuffer */.lW.isBuffer(b)) return undefined;
...
;
function isBinaryFormat(buf) {
  if (typeof buf === 'string') return buf.substring(0, 3) === 'BMF';
  return buf.length > 4 && equal(buf.slice(0, 4), is_binary_HEADER);
}
;
// EXTERNAL MODULE: ./node_modules/mime/index.js
var mime = __webpack_require__(893);
var mime_default = /*#__PURE__*/__webpack_require__.n(mime);
;// CONCATENATED MODULE: ./src/load-font.js
...
function parseFont(file, data, cb) {
  ...
  if (isBinaryFormat(data)) {
    ...
    binary = true;
  } else data = data.toString().trim(); // console.log(data);


  try {
    if (binary) result = readBMFontBinary(data);else if (/json/.test(mime_default().getType(file)) || data.charAt(0) === '{') result = JSON.parse(data);else if (/xml/.test(mime_default().getType(file)) || data.charAt(0) === '<') result = parseXML(data);else result = parseASCII(data);
  } catch (e) {
    cb(e);
    cb = noop;
  }
